...
async function importExercises() {
  try {
    ...
    // Ler o arquivo Excel
    const filePath = join(__dirname, "..", "Tabela Completa de Exercícios de Academia.xlsx");
    ...
    const exercises = data.map((row: any) => {
      ...
      const name = row["Nome"] || row["Exercício"] || row["nome"] || row["exercicio"];
      const muscleGroup = row["Grupo Muscular"] || row["Músculo"] || row["muscle_group"];
      const equipment = row["Equipamento"] || row["Equipamentos"] || row["equipment"];
      const description = row["Descrição"] || row["Execução"] || row["description"];
    ...
    for (let i = 0; i < exercises.length; i += batchSize) {
      ...
      if (error) {
        ...
        console.log(`✅ Lote ${Math.floor(i / batchSize) + 1}: ${data?.length} exercícios inseridos`);
      }
    }
    ...
    if (error.code === "ENOENT") {
      ...
      console.log("   2. Renomeie para: 'Tabela Completa de Exercícios de Academia.xlsx'");
      console.log("   3. Execute novamente: npx tsx scripts/import-exercises.ts");
    }
  }
}
